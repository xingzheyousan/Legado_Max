...
> **定位**：本文件是操作手册（怎么跑、命令与坑）；何时必须做 E2E、各层验收标准见规范 [docs/project-rules/e2e-testing-rules.md](../project-rules/e2e-testing-rules.md)；自动化框架选型见《E2E测试概念与自动化框架选型.md》。
...
三层验证**缺一不可已成文为强制规范**（`docs/project-rules/e2e-testing-rules.md`：触发时机与各层验收标准）；本手册负责各层的具体操作方法。传书 bug 的教训说明为什么不能省：**接口层**等任何一层的正确性都不能靠推测——最终根因（content-type 重写丢 boundary）只有接口层实测才能暴露。
